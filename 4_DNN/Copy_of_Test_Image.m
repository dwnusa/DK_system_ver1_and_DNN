...
IM2 = rot90(IM2,2);
figure, imagesc(IM2');
figure, scatterhist(before_XY(1,:), before_XY(2,:),'Kernel','on','MarkerSize',1);%'MarkerSize',1);
%%
IM = img22;
IM2 = imcomplement(IM);
IM2 = rot90(IM2,2);
figure, imagesc(IM2');
figure, scatterhist(after_XY(1,:), after_XY(2,:),'Kernel','on','MarkerSize',1);
%% 선형성 개선 확인
...
Y2 = sum(img22);
figure(6), 
subplot(4,1,1), plot(X1); title('X축');
subplot(4,1,2), plot(X2); title('X축');
subplot(4,1,3), plot(Y1); title('Y축');
subplot(4,1,4), plot(Y2); title('Y축');
